...
def _buscar_fundamentos_fiis_hg(pool, token):
    """Busca, em lotes, preço, variação, Dividend Yield (12m), valor de
    mercado e volume de todo o pool de FIIs via /v2/finance/quotes — a
    mesma chamada em lote já usada para ações, então cobre o pool inteiro
    numa passada só (a bolsai precisava de 2 requisições POR FII: uma
    para fundamentos, outra para volume).

    ATENÇÃO — aproximação conhecida: a HG não tem patrimônio líquido de
    FII (net_asset_value). Usamos market_cap (valor de MERCADO) como
    aproximação de patrimônio, por decisão explícita — ciente de que os
    dois números são conceitualmente diferentes (mercado reflete o preço
    da cota × cotas emitidas; patrimônio é o valor contábil apurado por
    laudo). Quando o fundo negocia com ágio ou deságio relevante sobre o
    valor patrimonial (P/VP ≠ 1), o ranking por este campo reflete o
    valor de mercado, não o patrimonial de fato."""
    ...
    for ativo in resultados_brutos:
        ...
        fundamentos[symbol] = {
            ...
            # Aproximação deliberada — ver docstring da função.
            "patrimonio": quote.get("market_cap"),
...
def coletar_rankings_fiis(token, pool=None, qtd=RANKING_FIIS_QTD):
    ...
    Valor Patrimonial (na verdade valor de MERCADO — ver ressalva abaixo),
    Maiores Dividend Yield e Mais Negociados (volume do dia).

    Usa a HG Brasil (1 chamada em lote para o pool inteiro). O rótulo
    "valor_patrimonial" no JSON de saída é mantido por compatibilidade com
    o front-end existente, mas o valor de fato é market_cap — ver a
    ressalva em _buscar_fundamentos_fiis_hg."""
    pool = pool or POOL_FIIS
    fundamentos = _buscar_fundamentos_fiis_hg(pool, token)
...
def coletar_ranking(token):
    ...
    mercado, receita) e 3 rankings de FIIs (valor de mercado — usado como
    aproximação de patrimonial, dividend yield, mais negociados) — 6 itens
    cada. Ambas as metades usam a HG Brasil (1 chamada em lote cada); se o
    token faltar, o robô pula as duas e mantém o ranking.json anterior."""
    ...
    if not token:
        print("AVISO: HGBRASIL_TOKEN não configurado — pulando rankings de ações e FIIs.", file=sys.stderr)
        return resultado
